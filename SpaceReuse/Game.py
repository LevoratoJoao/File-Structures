...
class Game:
    ...
    # Representacao do objeto (quando for printar)
    def __repr__(self):
        if(self.nome != "None"):
            return '' + self.nome + '|' + self.produtora + '|' + self.genero + '|' + self.plataforma + '|' + self.ano + '|' + self.classificacao + '|' + self.preco + '|' + self.midia + '|' + self.tamanho + ''
        else:
            return "None"

    # Retorna tamanho total do objeto (soma de cada atributo)
    def getSize(self):
    ...
    # Conta numero de atributos do objeto -> Se for diferente de None
